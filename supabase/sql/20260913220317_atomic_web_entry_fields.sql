-- AUD-13/15 B08c: extend the existing owner-only detail command with ordinary
-- web editor fields. Signature, invoker authority, locks and PT409 stay unchanged.
begin;
...
begin
  ...
  if exists(select 1 from jsonb_each(p_updates) field
    where field.key in ('entry_privacy','reaction_privacy','comments_privacy')
      and field.value not in ('"public"'::jsonb,'"friends"'::jsonb,'"private"'::jsonb))
