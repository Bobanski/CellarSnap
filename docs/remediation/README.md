...
**B04a/AUD-05 and B04b/AUD-07 are merged, deployed and verified; both findings are Closed for their stated scopes.** PRs #94/#95, final product release `c83a5cb`. Contact lookup is backend-only, auth/recovery use stable distributed limits, and remote wine-list downloads validate every destination with pinned DNS, a full-body deadline and byte caps. Exact migration/checksum, deployments and live evidence are in the [release handover](handovers/b04-release.md).

The current automated baseline is 250 passing tests. Desktop/phone browser and fresh Expo web auth/scanning QC passed, with explicit exceptions: native runtimes and actual recovery delivery/password changes were unavailable or not exercised. Production checks include 20 auth/authority assertions, six remote-menu assertions, phone rejection/filter interactions and desktop/phone results. Five disposable scans and 186 child wine rows were removed; the pre-existing scan was retained. QC-10 records an uncertain Expo recovery scroll gap; QC-11 records an existing successful-empty response for script-rendered menus. Neither is hidden by the release claims.

**Next bounded slice: AUD-08/B04c compatible dependency remediation.** Refresh advisory data before selecting upgrades. OPS-01 remains the duplicate Vercel project with missing Supabase environment. AUD-01 remains Partial for the measured cached-download revocation window and old-client behavior; public identity/ratings AUD-06/QC-01 remains a separate B02 follow-up. No native binary/OTA release was performed.
