...
- **Latest handover:** [B02b1 photo/group privacy](handovers/batch-02b1.md). Prior release: [B01/B02a SQL deployed](handovers/sql-rollout-b01-b02a.md); earlier [merged](handovers/merged-b01-b02a.md), [merge readiness](handovers/merge-readiness.md), [batch 02a](handovers/batch-02a.md) and [batch 01](handovers/batch-01.md).
...
B02b1 is implemented in [PR #86](https://github.com/Bobanski/CellarSnap/pull/86): photo metadata now has parent-plus-photo privacy policies, group metadata follows a valid readable anchor, individual slides check member/photo visibility, and web/mobile loaders filter missing parent references before signing. **The new migration is not deployed.** AUD-01 remains Partial; B02b2 still covers actual Storage/source/copy authority and anonymous share signing. [Contract](b02b1-photo-group-contract.md), [QC evidence](../audits/b02b1-photo-group-qc-2026-09-12.md).

At implementation `256a76f`, 189 automated tests, 54 isolated PostgreSQL/PostgREST HTTP checks, web/mobile lint/types, Next build and Expo export passed. Hands-on desktop/phone and Expo web verified owner three-photo and non-owner two-photo galleries plus private-entry denial. Browser checks used changed local code with unchanged live policies; post-rollout live SQL acceptance remains pending. No native runtime is installed, and Expo touch-gallery navigation remains unverified. Disposable fixtures were removed; account capability flags stayed unchanged. New QC-08 records an existing event-card detail-link mismatch, deferred to B06.

B01 [PR #80](https://github.com/Bobanski/CellarSnap/pull/80) and B02a [PR #82](https://github.com/Bobanski/CellarSnap/pull/82) remain merged, deployed and verified live. AUD-02/03 are Closed. Their SQL versions are `20260912211719` and `20260912211747`; exact reviewed-file checksums and rollout evidence are in the [prior handover](handovers/sql-rollout-b01-b02a.md). Do not reapply to reconcile timestamps. Primary production deployment succeeded previously; duplicate Vercel `cellarsnap` remains OPS-01. Continue B02b2 promptly, then public identity/ratings and urgent B03 personal knowledge isolation from the canonical backlog.
