...
**B04c compatible dependency remediation is implemented on `codex/b04c-dependencies`, PR #98 / issue #97, product `588d49a`; release is pending.** Next 16.3.5 and directly declared Sharp 0.35.4, Expo SDK 56 alignment and reviewed transitive patches clear the root audit (13 → 0) and all mobile high/critical entries (29 → 14 moderate). AUD-08 remains Partial for the legacy router decoder and xcode/uuid roots plus native/release acceptance. Exact reasoning and resume steps are in the [B04c handover](handovers/batch-04c.md).

The branch baseline is 256 passing isolated tests, web/mobile type/lint and Next production build. Expo web/iOS/Android Hermes exports passed, with actual desktop/phone web auth/scanning/image checks and Expo web auth/scanning checks recorded. Native simulators/emulators remain unavailable; exported native bytecode is not device acceptance or an app release. QC-10 was reproduced at a fixed phone viewport and is now Open for B11; existing public-rating, Maps and navigation/accessibility findings remain.

**Next: review/release B04c, then a bounded AUD-08 router-decoder compatibility repair.** Resolve the toolchain UUID path deliberately; don't force Expo downgrades. B05 remains the next broad schema/tooling batch. OPS-01 duplicate hosting, AUD-01 photo cache revocation/old-client acceptance and AUD-06/QC-01 public projections stay separate.

**Latest verified production remains B04a/AUD-05 and B04b/AUD-07, merged/deployed/live-verified at `c83a5cb`.** Those findings are Closed for their stated scopes. PRs #94/#95 and release-doc PR #96 are merged. The [release handover](handovers/b04-release.md) records SQL/checksums, 250 tests, 20 auth and six menu assertions, production desktop/phone checks and fixture cleanup. Do not treat the new dependency branch or preview as production deployment.
