...
- **Current feed checkpoint:** [B02k](handovers/batch-02k.md), [QC](evidence/b02k-qc.json): raw numeric ratings removed from the web feed transport; local QC passed, release pending. Broader QC-01 physical/mobile payloads remain open.
...
**B02g/B02h/B08c are merged through #134/#135/#136, product main `ae6f354`.** Adopted bearer/mobile clients now request-authorize photo bytes, and crop reads preserve original dimensions. Legacy raw/transformed URL revocation was measured on disposable objects; the production rekey/client cutoff remains required. Ordinary ungrouped web details and explicit grapes now save through the existing atomic command, with visible conflicts, safe retries and a deadline. Both forward SQL files are live and catalog-verified. [Release and resume steps](handovers/b02g-b02h-b08c-release.md).

**411 isolated checks, 18 schema tests**, actual PostgreSQL 17.6 replay/catalog and races, web/mobile types/lint, Next build and Expo exports passed. Actual desktop/phone web and Expo browser checks cover image revocation/retry/crop/sign-out, intentional fields/grapes/privacy, outage/lost-response retry, stale conflict and timeout. Release evidence distinguishes local and live results, expected faults and the existing not-ready scoring diagnostic. Native runtime/distribution remains unavailable and unreleased; fixture cleanup and original state are verified.

**Next:** P0 AUD-01 supported-client/native rollout and durable legacy rekey/authority cutoff; QC-01 private numeric projections; AUD-13/15 atomic group and remaining lifecycle commands. AUD-06 public-profile writes/raw names and helper disclosures are repaired; native acceptance remains. QC-07/QC-17 retain Partial statuses for native acceptance/distribution. AUD-10 ai_notes_summary/OCR/scoring, AUD-09 deferred badge facts/history and AUD-19/21 bootstrap/adoption remain Partial. New QC-18 unassociated web labels stays B11. #81/#104/#112 stay open. Merge/close permission applied only to the completed session and expires at handoff.
