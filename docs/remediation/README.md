...
- **Current handover:** [B11h iOS submission preparation](handovers/b11h-ios-submission-preparation.md), [sanitized evidence](evidence/b11h-ios-submission-preparation.json). Company/legal/privacy copy, submission metadata, release-art contracts, mobile support navigation and native picker metadata stripping are locally implemented and verified; merge and both web/static deployments remain pending. QC-25 is the remaining non-Apple App Review blocker: public UGC lacks reviewed server-authoritative filtering and an owned moderation-response operation. No native runtime is installed. Cluster Wine, LLC enrollment/invitation and company-owned credentials remain the external Apple gate. PR #172 historical retirement is merged but not hosted or run; zero production deletes and OPS-04 remains required. Queue **79 / 15 Closed**.
...
**B11h has a locally verified iOS submission-preparation candidate on `codex/ios-submission-readiness`.** Shared company terms, current public privacy disclosure, working mobile support navigation, deterministic release art, a complete listing/reviewer packet and native picker metadata stripping are implemented. 597 isolated tests, 49 schema/tool checks, web/mobile types/lint, release/dependency/tooling contracts, environment-backed web build, iOS production export and desktop/phone browser/Expo-web QC pass. No simulator/runtime is installed, and nothing is merged or deployed yet. QC-25 UGC filtering/moderation remains the next non-Apple code/ops blocker; five compatible Expo patch updates need a bounded review. [Current handover](handovers/b11h-ios-submission-preparation.md).
