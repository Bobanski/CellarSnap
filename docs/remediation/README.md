...
- **Latest handover:** [B03b knowledge lifecycle](handovers/batch-03b.md). Predecessor: [B03a knowledge read containment](handovers/batch-03a.md). Prior release: [B02b production rollout](handovers/sql-rollout-b02b.md). Implementation: [B02b2 Storage/share privacy](handovers/batch-02b2.md). Predecessor: [B02b1 photo/group privacy](handovers/batch-02b1.md). Prior release: [B01/B02a SQL deployed](handovers/sql-rollout-b01-b02a.md); earlier [merged](handovers/merged-b01-b02a.md), [merge readiness](handovers/merge-readiness.md), [batch 02a](handovers/batch-02a.md) and [batch 01](handovers/batch-01.md).
...
B03a / AUD-04 read isolation is merged in PR #90 (`19cb4fe`); SQL remote `20260913002605` is deployed and live read checks passed. B03b lifecycle/FK propagation is implemented with 210 tests and a rolled-back hosted migration rehearsal; rollout and live lifecycle QC are pending. [Current handover](handovers/batch-03b.md). AUD-04 is Partial.
...
Release QC passed 196 automated tests, 148 isolated Storage HTTP checks, 56 fresh live Storage/Data API assertions, five deployed API assertions, eight rolled-back SQL role assertions, and five share/OG states. Hands-on desktop/phone and Expo web owner/non-owner galleries and privacy denial passed. Hosted transformations are verified. Disposable data and capability fingerprints are restored. [Actual results and limits](../audits/b02b-production-rollout-qc-2026-09-12.md).

**AUD-01 remains Partial:** production QC measured a cached previously authorized SDK download surviving a privacy change, while fresh requests/new signatures correctly denied. Existing signed URLs keep their issued lifetimes. Define and verify the cache/revocation window and supported old-client behavior before closure. No native runtime is installed, and no native binary/OTA release was performed. This does not block the next independent urgent AUD-04/B03 knowledge-isolation slice.

B01 [PR #80](https://github.com/Bobanski/CellarSnap/pull/80) and B02a [PR #82](https://github.com/Bobanski/CellarSnap/pull/82) remain merged, deployed and verified live. AUD-02/03 are Closed. Their SQL versions are `20260912211719` and `20260912211747`; exact reviewed-file checksums and rollout evidence are in the [prior handover](handovers/sql-rollout-b01-b02a.md). Do not reapply to reconcile timestamps. Primary production deployment succeeded previously; duplicate Vercel `cellarsnap` remains OPS-01. B02b1/B02b2 release is complete; prioritize urgent AUD-04/B03 personal knowledge isolation next, with public identity/ratings (AUD-06/QC-01) as a separate B02 follow-up.
