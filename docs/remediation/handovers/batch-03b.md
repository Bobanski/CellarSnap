...
## Resume here
PR #91 is open. Lifecycle SQL is live as `20260913004127`, exact MD5 `190fa5195c0dd372edae429250596c33`. 20 live HTTP assertions, three true concurrent transactions and phone/Expo current-note checks passed; 386 current entries regenerated. Final bounded publication follow-up SQL and final-head release checks are next before merge. [QC report](../../audits/b03b-knowledge-lifecycle-qc-2026-09-12.md). Audit finding remains Partial until those release steps are verified.
...
## Verification
212 automated tests passed, including 16 actual pgvector/SQL/ingestion tests. Positive vulnerable baseline; read matrix; legacy writer rejection; composite ownership; owner search; client/RPC authority; synchronous entry/grape invalidation; stale publication; entry/account cascade; 103-entry pagination; embedding response errors; idempotence/replay. Web TypeScript/lint and production build passed. Hosted PostgreSQL 17.6 rehearsal executed exact SQL in a rolled-back transaction and verified curated retention, legacy invalidation, publication and update cleanup. Live races and post-migration phone/Expo QC passed. The initial per-entry writer took about a minute for 386 sources; forward migration `20260913004944_batch_personal_knowledge_publication.sql` reduces publication calls from 386 to four without dropping snapshot checks. This follow-up remains in AUD-26/B03b, and its live retest is next. B03a hands-on evidence remains in its [report](../../audits/b03a-knowledge-qc-2026-09-12.md).

## Release state
B03a merged and SQL deployed. B03b lifecycle SQL is migrated and personal vectors regenerated; PR #91 is not merged. The separate batching follow-up SQL remains pending. Keep the source authorization repair on rollback; forward-fix lifecycle functions/ingester. Do not restore old personal chunks into curated storage. New vectors can be regenerated from authoritative entries without changing ratings/notes.
