...
| AUD-21 | P1 | Database types drift from schema/domain DTOs | Partial | B05c issue #104 on `codex/b05c-database-types`: one generated source, typed web/mobile factory bridges and grape/producer browse query adoption. [Contract](b05-type-adoption.md). QC passed within [B05c scope](handovers/batch-05c.md); release pending. Unadopted clients/queries remain explicit. B05: generate one type source from reviewed schema, adopt client factories/query result typing, keep public DTOs/validators distinct and CI-check drift. |
