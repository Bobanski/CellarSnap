...
| AUD-05 | P1 | Identifier lookup exposes email/phone mappings | In progress | B04: non-enumerating public responses and restricted resolver authority; login/recovery and deliberate availability checks still work with abuse limits. |
...
| AUD-48 | P1 | Regression baseline incomplete/untrustworthy | Partial | Current B03 baseline: 212 tests with 16 actual pgvector/SQL/ingestion tests, 33 read/lifecycle HTTP assertions, three true concurrent cases, five production deletion/cleanup checks and desktop/phone/Expo QC. [Release](handovers/b03-release.md). Prior Storage/photo/access evidence remains in B01/B02 reports; hosted transformations verified in B02b rollout. Native and future import/cache/scoring coverage remain batch obligations. |
...
| AUD-50 | P2 | Silent degradation and stale engineering context | Partial | B03 adds checked ingestion publication outcomes and measured bounded request counts; hub/intake/handover continuity is maintained. QC-09 records ungrounded chat claims for investigation. Every batch: request/job IDs, stage latency/query counts, cache/fallback/AI usage and actionable error reporting remain implementation work. |
