...
Updated September 12, 2026. **60 records: all 50 original audit findings, eight browser/mobile QC findings, and two operational/reconciliation items.** This file is the work queue; do not maintain competing unchecked lists in successive handovers.
...
**Ownership / external tickets:** unassigned unless a row links a PR or active branch. B01 PR #80 and B02a PR #82 are merged/deployed. B02b1's photo/group metadata slice is [PR #86](https://github.com/Bobanski/CellarSnap/pull/86), tracked with [issue #81](https://github.com/Bobanski/CellarSnap/issues/81) on `codex/b02b1-photo-group-privacy`. B02b2 Storage/share authorization is [PR #87](https://github.com/Bobanski/CellarSnap/pull/87) on `codex/b02b2-storage-privacy`, stacked on #86 and referencing #81. There is no GitHub umbrella; these stable local IDs remain canonical. Issue #70 is closed brand/design work, not this backlog's umbrella.

**Release boundary:** AUD-02/03 are Closed following verified production rollout. AUD-01 remains Partial. B02b1 and B02b2 have passed their scoped application and isolated database/object-service QC, but neither is merged or deployed; hosted acceptance remains pending. The current live Storage broad-read policy is still present. “QC passed — release pending” never implies production remediation. Recheck current deployment state when resuming.

## Original audit

| ID | Priority | Finding | Status | Batch / next acceptance evidence |
|---|---|---|---|---|---|
| AUD-01 | P0 | Entry/photo policies bypass privacy | Partial | B02a entry-row migration deployed as `20260912211747`; live owner/non-owner API/web/Expo, photo metadata and rolled-back tester/block checks passed. [Rollout](handovers/sql-rollout-b01-b02a.md). B02b1 **QC passed — release pending** in [PR #86](https://github.com/Bobanski/CellarSnap/pull/86): photo metadata, anchor/member group policies and web/mobile pre-sign filtering. [Handover](handovers/batch-02b1.md). SQL not deployed; live SQL acceptance pending. B02b2 **QC passed — release pending** in [PR #87](https://github.com/Bobanski/CellarSnap/pull/87): source-authorized Storage and anonymous signing, reclassification/slide-only compatibility, fresh share authorization and one-hour share URLs. [Handover](handovers/batch-02b2.md). Apply both reviewed migrations, release code, verify hosted Storage/share/web/mobile flows, and assess already-issued URL lifetimes before closure. |
...
| AUD-48 | P1 | Regression baseline incomplete/untrustworthy | Partial | B02b2 expands to 196 tests, 148 real isolated Storage HTTP assertions and desktop/phone/Expo QC ([report](../audits/b02b2-storage-share-qc-2026-09-12.md)); hosted transformations/CDN/native remain unverified. B02b1 expands to 189 tests plus 54 new isolated photo/group HTTP checks and desktop/phone/Expo QC ([report](../audits/b02b1-photo-group-qc-2026-09-12.md)); prior 56 app/database comparisons, 41 B01/B02a HTTP checks and build/browser QC remain; rollout adds 35 live HTTP/Storage checks, eight rolled-back SQL assertions and live web/Expo checks. [Evidence](../audits/sql-rollout-qc-2026-09-12.md). Native and future lifecycle/import/cache/scoring coverage remain batch obligations. |
