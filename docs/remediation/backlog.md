...
| AUD-06 | P0 | Public identity projection overexposes fields and permits writes | Partial | B02c2 on `codex/b02c2-public-profile-authority`: hosted catalog confirms authenticated mutation grants and updatable identity columns on a definer view over owner-only profiles. Migration deployed `20260913195059`; all view writes denied, safe chosen names and visibility verified with hosted REST and eight web/Expo desktop/phone checks. PR #126 release pending. Legacy arbitrary-ID helper RPC disclosure and native coverage remain; numeric entry payloads stay QC-01. [Handover](handovers/batch-02c2.md). |
...
| QC-07 | P1 | Mobile entry details read an absent grape table | In progress | B08a uses typed canonical relation, blocks edits on read failure and skips grape writes on notes-only saves. QC-17 owner search fixed during actual QC; final retest pending. Atomic explicit mutation remains AUD-13. [Handover](handovers/batch-08a.md). |
...
| QC-17 | P1 | Mobile owner editor retains bulk-only search/error guards | In progress | B08a: actual notes-only/explicit-grape QC found search gated on bulk review. Enable it during normal owner editing and verify search/add/save/reload. |
...
- Priority / status: P1 / In progress; B08a with QC-07, AUD-13/21.
