...
|---|---|---|---|---|---|
| AUD-01 | P0 | Entry/photo policies bypass privacy | Partial | B02a/B02b1/B02b2 entry, metadata/group and Storage/share repairs are merged/migrated/deployed; release `fd391aa`. [Live QC](../audits/b02b-production-rollout-qc-2026-09-12.md): fresh authorization, signing, hosted transforms, desktop/phone and Expo galleries passed. Broad Storage read removed. B02b signed share URLs originally lasted one hour; B02d now uses an app image endpoint for new anonymous delivery. B02e QC passed — release pending on `codex/b02e-authenticated-web-images`: cookie-web request-time image delivery, 403 isolated checks and actual desktop/phone crop/privacy QC; bearer/native compatibility retained. [Checkpoint](handovers/batch-02e.md). Remaining: define/verify revocation window for previously authorized SDK CDN hits, old signed URLs and supported installed clients; native coverage unavailable. Preserve prior B02a/B02b1/B02b2 implementation history in linked handovers. B02d #127 merged: new anonymous images are reauthorized/no-store; old signed/SDK/transformed delivery remains open. [Current release](handovers/b02c2-b02d-b08a-release.md). |
...
| AUD-06 | P0 | Public identity projection overexposes fields and permits writes | Partial | B02c2 #126 merged `bff9cfd`, SQL `20260913195059` live: read-only chosen-name projection, raw names/email null, block/test filtering, owner reads/edits retained. Hosted REST, schema replay and eight web/Expo cases pass. B02f helper containment implemented on `codex/b02f-private-policy-helpers`; 405 isolated / 13 schema tests and PostgreSQL replay pass, hosted SQL/browser/Expo QC pending. [Checkpoint](handovers/batch-02f.md). Native coverage remains. Public numeric payloads stay QC-01. [Release](handovers/b02c2-b02d-b08a-release.md). |
...
| AUD-13 | P1 | Web/native entry lifecycle diverges | In progress | B08b atomic mobile detail/grape edits implemented on `codex/b08b-atomic-mobile-edits`; isolated failure/retry/concurrency pass. Hosted browser QC found `40001` left stale saves pending; forward correction uses explicit `PT409` and is being retested before release. [Checkpoint](handovers/batch-08b.md). B08: authoritative command/side-effect contract covering consumed/cellared/shared/group entries; parity across clients while preserving drafts and native I/O. |
...
| QC-17 | P1 | Mobile owner editor retains bulk-only search/error guards | Partial | B08a #128 merged `3415e6a`: normal owner search/add works, options survive scroll/keyboard blur, save errors appear beside Save changes. Desktop/phone failure/recovery passed. Native runtime/distribution remains. [Release](handovers/b02c2-b02d-b08a-release.md). |
