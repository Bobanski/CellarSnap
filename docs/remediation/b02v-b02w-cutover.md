...
2. Accept the native 1.0.1/build 2 candidates on actual devices against that staged backend. Cover login, library/events, feed/Home/profile/detail, normal and bulk creation, import/copy/drink, edits, photos/crop/avatars, retry and sign-out. Inspect errors and record device/OS/build IDs. Confirm the supported installed population and distribute the replacement. No OTA update is configured; these require binary distribution. The September source/build inventory cannot prove older March internal builds are unused.
