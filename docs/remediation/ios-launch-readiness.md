# iOS release readiness — September 23, 2026
...
- Current source candidate: main `5e992af` / #170, Expo SDK 57.0.24 / React Native 0.86.3, production prebuild template `expo-template-bare-minimum@57.0.26`. The upgrade removes the known SDK 56 Hermes V1 memory-regression exposure; Expo Doctor reports 21/21 checks passing.
...
- Store-build attempt at merged main `5e992af`: production EAS environment and remote credentials resolved, then non-interactive setup stopped before job creation because the distribution certificate is not validated and credentials require interactive Apple login/2FA. No EAS build ID, IPA, App Store upload or App Review submission. Complete Apple authentication and production provisioning, then rebuild this exact tree.
...
- Privacy candidate: #167/#168/#170 are merged; main `5e992af` includes account-wide AI permission/disclosures, the SDK 57 candidate and native declaration hardening. Matching backend/web deployed and production desktop/phone web + Expo-browser checks passed. ExifReader 4.41.1 patches a fresh advisory. [Current handover](handovers/b02y-archive-release.md).
