...
## Production release verification — September 12, 2026 (UTC September 13)

PR #95 merged as `c83a5cb9e8cfb5c94ed950e1ea0edc50ee33e309`. Both CI jobs and primary preview passed on head `0c87f55`; duplicate Vercel failed with the existing missing Supabase environment error (OPS-01). Primary production `dpl_3xyryoBujFvufLxw7Gf3aibBChDY` became Ready with `cellarsnap.app` alias. Its build log explicitly identifies main commit `c83a5cb`.

Five disallowed inputs returned 422 (IPv4/IPv6 loopback, Azure platform VIP, embedded credentials and port 5432). A real Maison Harlem URL returned 48 wines. [Sanitized API evidence](../remediation/evidence/b04b-live-results.json). Production browser at 390×844 showed the private-address error and enabled retry/clear controls; saved results rendered 48/48 and three recommendations. Opening filters and deselecting White changed the count to 33/48; Done closed the picker. Reload at 1440×1000 restored 48/48 and displayed the recommendations without horizontal overflow. Screenshots were saved and visually inspected at `/tmp/cellarsnap-b04-qc/production-menu-private-phone.png`, `production-menu-results-phone.png`, and `production-menu-results-desktop.png`.

The initial automated fill immediately after navigation was lost during hydration; retry on the settled form passed. The browser logged the expected `Scan failed` warning for the intentional 422 and no new error. Vercel error-level runtime log query for this deployment from 02:28 UTC through 02:30 UTC returned zero records; this is a bounded observation, not a claim about all future traffic. Five captured QC scans and 186 child wines were removed; the original scan remained. [Cleanup evidence](../remediation/evidence/b04-cleanup-results.json). Local servers stopped and browser viewport override cleared. Native and recovery-delivery limitations remain as above.
