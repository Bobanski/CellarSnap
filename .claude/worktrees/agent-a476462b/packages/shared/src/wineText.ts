const UNKNOWN_WINE_NAME_VALUES = new Set([
  "unknown",
  "unknown wine",
  "n/a",
  "na",
...
export function normalizeWineText(value: unknown) {
...
export function isUnknownWineName(value: unknown) {
  const normalized = toNullableTrimmedString(value);
  if (!normalized) {
    return false;
  }
  return UNKNOWN_WINE_NAME_VALUES.has(normalized.toLowerCase());
}
