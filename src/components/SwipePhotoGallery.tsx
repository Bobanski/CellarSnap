...
export default function SwipePhotoGallery({
  ...
  return (
    <div
      className={`relative overflow-hidden rounded-3xl border border-white/10 bg-black/40 ${wrapperClassName}`}
    >
      <div
      ...
      >
        {items.map((item, itemIndex) => (
          <div
            key={item.id ?? `${item.badge ?? "photo"}-${item.url ?? "missing"}-${itemIndex}`}
