...
import { resolvePublicShareSource, type EntryRow } from "@/server/shares/source";
import { publicShareImageUrl } from "@/server/shares/imageDelivery";
...
async function resolvePublicPostShareUncached(
  ...
  const {share, entry, profile} = source;
  const [{labelPath, previewPath}, {data: rawEntryScore}] = await Promise.all([
  ...
  const authorName = getPublicProfileName(profile, "Someone from Cluster");
  ...
  const labelImageUrl = labelPath ? publicShareImageUrl(share.id, "label") : null;
  const previewImageUrl = previewPath ? publicShareImageUrl(share.id, "preview") : null;
  const labelImageOgUrl = labelPath ? publicShareImageUrl(share.id, "label", "og") : null;
  const previewImageOgUrl = previewPath ? publicShareImageUrl(share.id, "preview", "og") : null;
...
// authorization decisions across requests. Image requests independently recheck it.
