...
import { resolvePostSaveSurveyTransition } from "@shared/entry-flow";
import { classifyPhotoTypeWithAi } from "@/lib/entryFlow/web/photoTypeClassifier";
...
export default function EditEntryPage() {
  ...
  const classifyPhotoType = async (file: File): Promise<EntryPhotoType> => {
    ...
    return classifyPhotoTypeWithAi(contextFile);
  };
  ...
  const saveCrop = async () => {
    ...
    try {
      ...
      const originalPath = buildOriginalPhotoPath(cropEditorPhoto.path);
  ...
  const submitBulkSurvey = async (submission: PostSaveSurveySubmission) => {
    ...
    try {
      const result = await submitPostSaveSurveyRequest({
        supabase,
        entryId: pendingBulkSurvey.entry.id,
        answers: submission,
      });
  ...
  const submitBulkComparison = async (response: ComparisonResponse) => {
    ...
    try {
      const result = await submitPostSaveSurveyRequest({
        supabase,
        entryId: pendingBulkSurvey.entry.id,
        answers: pendingBulkSurvey.surveyAnswers,
        comparisonEntryId: pendingBulkSurvey.candidate.id,
        response,
      });
    ...
    <AppShell>
      ...
      <div className="mx-auto w-full max-w-3xl space-y-8">
        ...
        >
                    ...
                    <>
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        {activePhoto ? (
                          <button
                            type="button"
                            className="rounded-full border border-[var(--color-border)] px-2 py-1 text-[10px] uppercase tracking-[0.12em] text-[var(--color-text-primary)] transition hover:border-[var(--color-error)] hover:text-[var(--color-error)] disabled:cursor-not-allowed disabled:opacity-60"
                            onClick={() => {
                        ...
                        <div className="grid gap-3 sm:grid-cols-[7rem_minmax(0,1fr)]">
                          ...
                          <div className="space-y-3">
                            <div className="flex flex-wrap items-start justify-between gap-2">
                              ...
                              <button
                                type="button"
                                className="rounded-full border border-[var(--color-error)]/40 px-3 py-1 text-xs font-semibold text-[var(--color-error)] transition hover:border-[var(--color-error)] disabled:cursor-not-allowed disabled:opacity-60"
                                disabled={saving}
          ...
          {photoError ? (
            <p className="text-sm text-[var(--color-error)]">{photoError}</p>
          ) : null}
          ...
          >
            ...
            {isBulkReview ? (
              <button
                type="button"
                className="rounded-full border border-[var(--color-error)]/40 px-4 py-2 text-sm font-semibold text-[var(--color-error)] transition hover:border-[var(--color-error)] disabled:cursor-not-allowed disabled:opacity-60"
