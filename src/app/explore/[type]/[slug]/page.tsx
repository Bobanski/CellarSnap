...
const CHAMPAGNE = "#F5EDD6"; // used only for light-on-dark text inside the photo Hero scrim, kept dark by design
...
const VERDOT = "#3D6B4F";
const VIOGNIER = "#8F6A1E"; // Viognier gold, darkened — only ever used as text here, and full-brightness gold fails contrast as text
const BG_ODD = "#F5EDD6"; // Champagne — light alternating section background
const BG_EVEN = "#FFFFFF"; // White — light alternating section background
const DEVICE_BG = "#F5EDD6"; // Champagne — page background
  ...
  return (
    <svg width={size} height={size} viewBox={`0 0 ${vb} ${vb}`} className="mx-auto">
      {[0.33, 0.66, 1.0].map((level) => (
        <polygon key={level} points={radarPolygon(cx, cy, maxR * level)} fill="none" stroke="rgba(44,26,14,0.08)" strokeWidth="0.5" />
      ))}
      {AXES.map((_, i) => {
        const [x, y] = radarPoint(cx, cy, maxR, i);
        return <line key={i} x1={cx} y1={cy} x2={x} y2={y} stroke="rgba(44,26,14,0.08)" strokeWidth="0.5" />;
      })}
      ...
      {AXES.map((label, i) => {
        ...
        return <text key={label} x={x} y={y} fill={FOG} fontSize="10" textAnchor={anchor} dominantBaseline="central">{label}</text>;
      })}
    </svg>
  );
}
...
function StoryTitle({ children }: { children: React.ReactNode }) {
  return <p style={{ fontFamily: SERIF, fontSize: 16, color: TERROIR, marginBottom: 8 }}>{children}</p>;
}
...
function QprBar({ qpr, label }: { qpr: CommunityQpr; label: string }) {
  ...
  return (
    <>
      <p style={{ fontFamily: SANS, fontSize: 9, color: FOG, marginBottom: 5 }}>{label}</p>
      ...
      {hasProducers ? (
        <div style={{ display: "flex", gap: 6, marginTop: hasQpr ? 10 : 0 }}>
          {c.most_loved_producer ? (
            <Link href={exploreHref("producer", c.most_loved_producer.name)} style={{ flex: 1, background: "rgba(44,26,14,0.04)", borderRadius: 8, padding: "8px 10px", textDecoration: "none" }}>
              <p style={{ fontFamily: SERIF, fontSize: 12, color: TERROIR, marginBottom: 2 }}>{c.most_loved_producer.name}</p>
              <p style={{ fontFamily: SANS, fontSize: 9, color: FOG }}>Most loved · {c.most_loved_producer.avg_rating} avg</p>
            </Link>
          ) : null}
          {c.best_qpr_producer ? (
            <Link href={exploreHref("producer", c.best_qpr_producer.name)} style={{ flex: 1, background: "rgba(44,26,14,0.04)", borderRadius: 8, padding: "8px 10px", textDecoration: "none" }}>
              <p style={{ fontFamily: SERIF, fontSize: 12, color: TERROIR, marginBottom: 2 }}>{c.best_qpr_producer.name}</p>
              <p style={{ fontFamily: SANS, fontSize: 9, color: FOG }}>{bestQprSub}</p>
            </Link>
          ) : null}
        </div>
      ) : null}
  ...
  return (
    <Section bg={bg}>
      ...
      <div style={{ display: "flex", gap: 6 }}>
        {picks.map((rec) => (
          <Link key={rec.name} href={exploreHref(rec.type, rec.name)} style={{ flex: 1, background: "rgba(44,26,14,0.04)", borderRadius: 8, padding: "9px 8px", border: "0.5px solid rgba(44,26,14,0.06)", textDecoration: "none" }}>
            <p style={{ fontFamily: SERIF, fontSize: 12, color: TERROIR, marginBottom: 3 }}>{rec.name}</p>
            <p style={{ fontFamily: SANS, fontSize: 9, color: FOG, lineHeight: 1.4 }}>{rec.why}</p>
          </Link>
        ))}
      </div>
    </Section>
  ...
  return (
    <Section bg={bg}>
      ...
      <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
        {items.map((item) => (
          <span key={item} style={{ background: "rgba(44,26,14,0.06)", borderRadius: 20, padding: "5px 11px", fontFamily: SANS, fontSize: 10, color: FOG, border: "0.5px solid rgba(44,26,14,0.08)" }}>
            {item}
          </span>
        ))}
      </div>
    ...
    <div style={{ background: bg, borderBottom: noBorder ? "none" : `0.5px solid ${SECTION_BORDER}`, padding: "14px 16px" }}>
      ...
      {facts.map((fact, i) => (
        <div key={i} style={{ display: "flex", gap: 8, marginBottom: 8 }}>
          ...
          <span style={{ fontFamily: SANS, fontSize: 10, color: FOG, lineHeight: 1.55 }}>{fact}</span>
        </div>
      ))}
    </div>
  );
  ...
  return (
    <>
      ...
      <Section bg={nextBg()}>
        ...
        {hasLogs ? (
          <>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", background: "rgba(44,26,14,0.05)", borderRadius: 10, padding: 12, marginBottom: 8 }}>
              <div>
                <p style={{ fontFamily: SANS, fontSize: 8, color: FOG, letterSpacing: 1, textTransform: "uppercase", marginBottom: 3 }}>TOP RATED</p>
                <p style={{ fontFamily: SERIF, fontSize: 14, color: TERROIR }}>{personal_stats.entry_count} {personal_stats.entry_count === 1 ? "wine" : "wines"} logged</p>
              </div>
            ...
            <div style={{ background: "rgba(196,96,122,0.1)", borderRadius: 8, padding: "9px 12px", borderLeft: "2px solid rgba(196,96,122,0.5)", marginBottom: 8 }}>
              <p style={{ fontFamily: SANS, fontSize: 11, color: TERROIR, lineHeight: 1.55 }}>
      ...
      {flavorProfile ? (
        <Section bg={nextBg()}>
          ...
          <div style={{ display: "flex", justifyContent: "center", gap: 16, marginTop: 8 }}>
            {hasLogs ? <div style={{ display: "flex", alignItems: "center", gap: 6 }}><div style={{ width: 14, height: 2, borderRadius: 1, background: ROSE }} /><span style={{ fontFamily: SANS, fontSize: 9, color: FOG }}>Your {personal_stats.entry_count} {personal_stats.entry_count === 1 ? "log" : "logs"}</span></div> : null}
            <div style={{ display: "flex", alignItems: "center", gap: 6 }}><div style={{ width: 14, height: 2, borderRadius: 1, background: accent }} /><span style={{ fontFamily: SANS, fontSize: 9, color: FOG }}>Region avg</span></div>
          </div>
          <p style={{ fontFamily: SANS, fontSize: 10, color: FOG, lineHeight: 1.55, marginTop: 8, textAlign: "center" }}>
      ...
      {storyText ? (
        <Section bg={nextBg()}>
          ...
          <p style={{ fontFamily: SANS, fontSize: 11, color: TERROIR, lineHeight: 1.75, marginBottom: 10 }}>{storyText}</p>
          {funFacts.length > 0 ? (
            <div style={{ background: "rgba(196,96,122,0.07)", borderRadius: 8, padding: "10px 12px", border: "0.5px solid rgba(196,96,122,0.2)" }}>
              ...
              <p style={{ fontFamily: SANS, fontSize: 11, color: FOG, lineHeight: 1.6 }}>{funFacts[0]}</p>
            </div>
          ) : null}
        </Section>
      ) : null}
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: 5 }}>
            {grapeItems.map((grape) => (
              <Link key={grape.name} href={exploreHref("grape", grape.name)} style={{
                ...
                background: grape.primary ? "rgba(123,29,58,0.35)" : "rgba(44,26,14,0.05)",
                border: grape.primary ? "0.5px solid rgba(196,96,122,0.3)" : "0.5px solid rgba(44,26,14,0.08)",
              }}>
                <span style={{ fontFamily: SANS, fontSize: grape.primary ? 10 : 9, color: grape.primary ? TERROIR : FOG, fontWeight: grape.primary ? 500 : 400 }}>{grape.name}</span>
              </Link>
            ))}
          </div>
          <p style={{ fontFamily: SANS, fontSize: 9, color: FOG, marginTop: 8 }}>Primary grapes highlighted · tap any to explore</p>
        </Section>
      ) : null}
        ...
        <Section bg={nextBg()}>
          ...
          {winemakerItems.map((wm) => (
            <Link key={wm.name} href={exploreHref("producer", wm.name)} style={{ display: "flex", gap: 10, marginBottom: 8, alignItems: "flex-start", textDecoration: "none" }}>
              ...
              <div>
                <p style={{ fontFamily: SERIF, fontSize: 13, color: TERROIR, marginBottom: 2 }}>{wm.name}</p>
                <p style={{ fontFamily: SANS, fontSize: 10, color: FOG, lineHeight: 1.4 }}>{wm.why}</p>
              </div>
            </Link>
          ))}
        </Section>
        ...
        <Section bg={nextBg()}>
            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: 5, marginBottom: zones.length > 0 ? 10 : 0 }}>
              {appellationItems.map((app) => (
                <Link key={app.name} href={exploreHref("region", app.name)} style={{ borderRadius: 20, padding: "4px 10px", background: "rgba(123,29,58,0.35)", border: "0.5px solid rgba(196,96,122,0.3)", textDecoration: "none" }}>
                  <span style={{ fontFamily: SANS, fontSize: 10, color: TERROIR, fontWeight: 500 }}>{app.name}</span>
                </Link>
              ))}
            </div>
          ) : null}
          {zones.map((zone) => (
            <div key={zone.name} style={{ marginBottom: 7 }}>
              <p style={{ fontFamily: SANS, fontSize: 10, color: TERROIR, fontWeight: 500, marginBottom: 2 }}>{zone.name}</p>
              {zone.note ? <p style={{ fontFamily: SANS, fontSize: 9, color: FOG, lineHeight: 1.45 }}>{zone.note}</p> : null}
            </div>
          ))}
        </Section>
      ) : null}
        ...
        <Section bg={nextBg()}>
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: 5 }}>
            {c.related_regions.map((name) => (
              <Link key={name} href={exploreHref("region", name)} style={{ borderRadius: 20, padding: "4px 10px", background: "rgba(44,26,14,0.05)", border: "0.5px solid rgba(44,26,14,0.08)", textDecoration: "none" }}>
                <span style={{ fontFamily: SANS, fontSize: 9, color: FOG }}>{name}</span>
              </Link>
            ))}
          </div>
        </Section>
    ...
    <>
        ...
        {hasLogs ? (
          <>
            <div style={{ display: "flex", gap: 8 }}>
              <div style={{ flex: 1, background: "rgba(44,26,14,0.04)", borderRadius: 8, padding: "8px 10px" }}>
                <p style={{ fontFamily: SERIF, fontSize: 20, color: TERROIR }}>{personal_stats.entry_count}</p>
                <p style={{ fontFamily: SANS, fontSize: 8, color: FOG }}>wines logged</p>
              </div>
              {personal_stats.avg_rating > 0 ? (
                <div style={{ flex: 1, background: "rgba(44,26,14,0.04)", borderRadius: 8, padding: "8px 10px" }}>
                  <p style={{ fontFamily: SERIF, fontSize: 20, color: VIOGNIER }}>{personal_stats.avg_rating.toFixed(1)}</p>
                  <p style={{ fontFamily: SANS, fontSize: 8, color: FOG }}>your avg rating</p>
                </div>
              ) : null}
              <div style={{ flex: 1, background: "rgba(44,26,14,0.04)", borderRadius: 8, padding: "8px 10px" }}>
                <p style={{ fontFamily: SERIF, fontSize: 20, color: TERROIR }}>—</p>
                <p style={{ fontFamily: SANS, fontSize: 8, color: FOG }}>community avg</p>
              </div>
            </div>
            <p style={{ fontFamily: SANS, fontSize: 10, color: FOG, lineHeight: 1.5, marginTop: 6 }}>
          ...
          <div style={{ background: "rgba(196,96,122,0.1)", borderRadius: 8, padding: "9px 12px", borderLeft: "2px solid rgba(196,96,122,0.5)" }}>
            <p style={{ fontFamily: SANS, fontSize: 11, color: TERROIR, lineHeight: 1.55 }}>You haven&apos;t logged any {profile.display_name} wines yet. Start exploring this grape to discover your personal preferences.</p>
          </div>
        )}
      </Section>
      ...
      {flavorProfile ? (
        <Section bg={nextBg()}>
          ...
          <FlavorRadar data={flavorProfile} accentColor={accent} size={200} />
          <div style={{ display: "flex", justifyContent: "center", gap: 16, marginTop: 8 }}>
            {hasLogs ? <div style={{ display: "flex", alignItems: "center", gap: 6 }}><div style={{ width: 14, height: 2, borderRadius: 1, background: ROSE }} /><span style={{ fontFamily: SANS, fontSize: 9, color: FOG }}>Your {personal_stats.entry_count} logs</span></div> : null}
            <div style={{ display: "flex", alignItems: "center", gap: 6 }}><div style={{ width: 14, height: 2, borderRadius: 1, background: accent }} /><span style={{ fontFamily: SANS, fontSize: 9, color: FOG }}>{profile.display_name} avg</span></div>
          </div>
          {c.personal_insight ? <p style={{ fontFamily: SANS, fontSize: 9, color: FOG, textAlign: "center", marginTop: 4 }}>{c.personal_insight}</p> : null}
        </Section>
      ) : null}
      ...
      {storyText ? (
        <Section bg={nextBg()}>
          <p style={{ fontFamily: SERIF, fontSize: 16, color: TERROIR, lineHeight: 1.5, marginBottom: 8 }}>{storyText}</p>
          {funFacts.length > 0 ? (
            <div style={{ display: "flex", gap: 8, alignItems: "flex-start" }}>
              <span style={{ color: accent, fontFamily: SANS, fontSize: 9, flexShrink: 0, marginTop: 1 }}>✦</span>
              <span style={{ fontFamily: SANS, fontSize: 10, color: FOG, lineHeight: 1.5 }}>{funFacts[0]}</span>
            </div>
          ) : null}
        </Section>
      ) : null}
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: 5 }}>
            {whereItGrows.map((region) => (
              <Link key={region.name} href={exploreHref("region", region.name)} style={{
                borderRadius: 20, background: "rgba(44,26,14,0.06)", border: "0.5px solid rgba(44,26,14,0.08)", textDecoration: "none",
                padding: region.size === "large" ? "6px 14px" : region.size === "medium" ? "5px 11px" : "4px 9px",
              }}>
                <span style={{ fontFamily: SANS, fontSize: region.size === "large" ? 11 : region.size === "medium" ? 10 : 9, color: region.size === "large" ? TERROIR : FOG, fontWeight: region.size === "large" ? 500 : 400 }}>{region.name}</span>
              </Link>
            ))}
          </div>
        </Section>
      ...
      {stylesExpressions.length > 0 ? (
        <Section bg={nextBg()}>
          ...
          {stylesExpressions.map((expr) => (
            <div key={expr.style} style={{ background: "rgba(44,26,14,0.04)", borderRadius: 8, padding: "10px 12px", marginBottom: 6 }}>
              <p style={{ fontFamily: SERIF, fontSize: 13, color: TERROIR, marginBottom: 3 }}>{expr.style}</p>
              <p style={{ fontFamily: SANS, fontSize: 9, color: FOG, lineHeight: 1.45, marginBottom: 3 }}>{expr.desc}</p>
              <p style={{ fontFamily: SANS, fontSize: 8, color: FOG }}>{expr.example}</p>
            </div>
          ))}
        </Section>
      ) : null}
        ...
        <Section bg={nextBg()}>
          ...
          {notableProducers.map((prod) => (
            <Link key={prod.name} href={exploreHref("producer", prod.name)} style={{ display: "flex", gap: 8, marginBottom: 8, alignItems: "flex-start", textDecoration: "none" }}>
              ...
              <div>
                <p style={{ fontFamily: SERIF, fontSize: 12, color: TERROIR }}>{prod.name}</p>
                <p style={{ fontFamily: SANS, fontSize: 9, color: FOG, lineHeight: 1.4 }}>{prod.note}</p>
              </div>
            </Link>
          ))}
        </Section>
  ...
  return (
    <>
      ...
      <Section bg={nextBg()}>
        <SectionLabel color={ROSE}>YOUR {profile.display_name.toUpperCase()}</SectionLabel>
        {hasLogs ? (
          <p style={{ fontFamily: SANS, fontSize: 10, color: FOG, lineHeight: 1.6 }}>
            You&apos;ve opened <span style={{ color: TERROIR, fontWeight: 600 }}>{personal_stats.entry_count} {personal_stats.entry_count === 1 ? "bottle" : "bottles"}</span>
          ...
          <div style={{ background: "rgba(196,96,122,0.1)", borderRadius: 8, padding: "9px 12px", borderLeft: "2px solid rgba(196,96,122,0.5)" }}>
            <p style={{ fontFamily: SANS, fontSize: 11, color: TERROIR, lineHeight: 1.55 }}>You haven&apos;t logged any {profile.display_name} wines yet. Open your first bottle to start tracking.</p>
          </div>
        )}
      </Section>
        ...
        <Section bg={nextBg()}>
          ...
          {philosophyTags.map((item) => (
            <div key={item.tag} style={{ marginBottom: 8 }}>
              <p style={{ fontFamily: SANS, fontSize: 10, color: TERROIR, fontWeight: 500, marginBottom: 2 }}>{item.tag}</p>
              <p style={{ fontFamily: SANS, fontSize: 9, color: FOG, lineHeight: 1.45 }}>{item.note}</p>
            </div>
          ))}
        </Section>
      ) : null}
      ...
      {keyWines.length > 0 ? (
        <Section bg={nextBg()}>
          ...
          {keyWines.map((wine) => (
            <div key={wine.name} style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", background: "rgba(44,26,14,0.04)", borderRadius: 8, padding: "10px 12px", marginBottom: 6 }}>
              <div>
                <p style={{ fontFamily: SERIF, fontSize: 13, color: TERROIR, marginBottom: 3 }}>{wine.name}</p>
                {wine.desc ? <p style={{ fontFamily: SANS, fontSize: 9, color: FOG, lineHeight: 1.4 }}>{wine.desc}</p> : null}
              </div>
        ...
        <Section bg={nextBg()}>
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: 5 }}>
            {regionGrapes.map((name) => (
              <Link key={name} href={exploreHref(name.includes("-") || (name.includes(" ") && name.length > 12) ? "region" : "grape", name)} style={{ borderRadius: 20, padding: "5px 11px", background: "rgba(44,26,14,0.06)", border: "0.5px solid rgba(44,26,14,0.08)", textDecoration: "none" }}>
                <span style={{ fontFamily: SANS, fontSize: 10, color: FOG }}>{name}</span>
              </Link>
            ))}
          </div>
        </Section>
        ...
        <Section bg={nextBg()}>
          ...
          <div style={{ display: "flex", gap: 6 }}>
            {similarProducers.map((prod) => (
              <Link key={prod.name} href={exploreHref("producer", prod.name)} style={{ flex: 1, background: "rgba(44,26,14,0.04)", borderRadius: 8, padding: "9px 8px", border: "0.5px solid rgba(44,26,14,0.06)", textDecoration: "none" }}>
                <p style={{ fontFamily: SERIF, fontSize: 12, color: TERROIR, marginBottom: 3 }}>{prod.name}</p>
                <p style={{ fontFamily: SANS, fontSize: 9, color: FOG, lineHeight: 1.4 }}>{prod.why}</p>
              </Link>
            ))}
          </div>
        </Section>
  ...
  if (loading) {
    return (
      <div style={{ background: DEVICE_BG, minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center" }}>
        <div className="h-8 w-8 animate-spin rounded-full border-2 border-[rgba(44,26,14,0.2)] border-t-[#C4607A]" />
      </div>
    );
  }
  ...
  if (error || !data) {
    return (
      <div style={{ background: DEVICE_BG, minHeight: "100vh", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 12, padding: 24 }}>
        <p style={{ fontFamily: SERIF, fontSize: 20, color: TERROIR, textAlign: "center" }}>Unable to load profile</p>
        <p style={{ fontSize: 13, color: FOG, textAlign: "center" }}>{error ?? "Something went wrong."}</p>
        <button onClick={() => router.back()} style={{ fontFamily: SANS, fontSize: 13, color: FOG, fontWeight: 600, background: "none", border: "none", cursor: "pointer", padding: 8 }}>Go back</button>
      </div>
    );
  }
  ...
  return (
    <div style={{ background: DEVICE_BG, minHeight: "100vh", maxWidth: 600, margin: "0 auto" }}>
      ...
      {data.profile.hero_image_attribution ? (
        <p style={{ fontSize: 10, color: FOG, textAlign: "center", padding: "16px 18px 32px" }}>
